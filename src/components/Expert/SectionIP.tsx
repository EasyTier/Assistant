      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormField label={t('ipv4Address')} htmlFor="ipv4" description={t('requiredWhenDhcpOff')}>
          <input
            ...
            className="w-full px-3 py-2 rounded-lg border border-[var(--color-border)] dark:border-[var(--color-border-dark)] bg-[var(--color-bg)] dark:bg-[var(--color-bg-dark)] text-sm text-[var(--color-text-h)] dark:text-[var(--color-text-h-dark)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] dark:focus:ring-[var(--color-accent-dark)] transition-shadow disabled:opacity-50"
          />
        </FormField>
